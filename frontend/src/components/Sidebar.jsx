...
const Sidebar = ({ profile, onLogout }) => {
  ...
  const patientLinks = [
    ...
    { name: 'Symptoms',         path: '/symptoms',         icon: <Activity size={19} /> },
    { name: 'Reports',          path: '/reports',          icon: <FileText size={19} /> },
    { name: 'Scan Prescription',path: '/prescription-scan',icon: <ScanLine size={19} /> },
    { name: 'Reminders',        path: '/reminders',        icon: <Bell size={19} /> },
    ...
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav" style={{ flex: 0 }}>
        ...
        <NavLink
          ...
          className={({ isActive }) => `nav-link ${isActive ? 'nav-link-active' : ''}`}
        >
          <span className="nav-icon"><SettingsIcon size={19} /></span>
      ...
      <style>{`
        ...
        .nav-link-active::before { background: transparent !important; }
